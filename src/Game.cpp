...
#define RESERVED_FLASH_START                                                   \
	((2 * 1024 * 1024) - RESERVED_FLASH_SIZE                                   \
	) // 2MB flash, reserved 32kB for NVMem
const uint8_t *flash_target_contents =
	(const uint8_t *)(XIP_BASE + RESERVED_FLASH_START);
...
void loadGames() {
	...
	if (ctx.nvmem.magic != NVMEM_MAGIC) {
		...
		printf(
			"[DAO] Loaded NVMem with %d players and %d games\n",
...
void saveGames() {
	...
	int rc = flash_safe_execute(
		call_flash_range_erase,
		(void *)RESERVED_FLASH_START,
		UINT32_MAX
	);
...
uint8_t FotoToL(uint16_t fotoValue) {
	return clamp<int32_t>(
		map<int32_t>((int32_t)fotoValue, FOTO_MIN, FOTO_MAX, 8, 128),
		8,
		128
	);
}