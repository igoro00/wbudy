...
BaseType_t rename_command(
	...
	if (strlen(name_param) >= (PLAYER_NAME_SIZE * 4)) {
		snprintf(
			write_buffer,
			write_buffer_len,
			"Name too long, max length is %dB\n",
	...
	for (uint32_t i = 0; i < ctx.nvmem.currentPlayer; i++) {
		if (ctx.nvmem.players[i].uid == uid) {
			...
			ctx.nvmem.players[i].name[(PLAYER_NAME_SIZE * 4) - 1] =
				'\0'; // Ensure null termination
			snprintf(
				write_buffer,
				write_buffer_len,
				"Renamed player (UID:0x%08X) to \"%s\"\n",
				uid,
				name_param
			);
			return pdFALSE;
	...
	ctx.nvmem.players[ctx.nvmem.currentPlayer]
		.name[(PLAYER_NAME_SIZE * 4) - 1] = '\0'; // Ensure null termination
	ctx.nvmem.currentPlayer++;
	snprintf(
		write_buffer,
		write_buffer_len,
		"Created player (UID:0x%08X) with name \"%s\"\n",
		uid,
		name_param
	);
